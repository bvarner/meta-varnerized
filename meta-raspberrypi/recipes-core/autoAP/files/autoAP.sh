...
configure_ap () {
    if [ -e /etc/systemd/network/08-wlan-client.network ]; then
        mv /etc/systemd/network/08-wlan-client.network /etc/systemd/network/08-wlan-client.network~
        systemctl restart systemd-networkd
    fi
}

configure_client () {
    if [ -e /etc/systemd/network/08-wlan-client.network~ ] &&  wpa_cli -i$device status | grep -q "mode=station"; then
        mv /etc/systemd/network/08-wlan-client.network~ /etc/systemd/network/08-wlan-client.network
        systemctl restart systemd-networkd
    fi
}
